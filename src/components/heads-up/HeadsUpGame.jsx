...
import { createRound, recordGesture } from "@/lib/heads-up/gameEngine.mjs";
import { useTiltGesture } from "@/hooks/useTiltGesture";

const COPY = {
  es: { title: "En la frente", choose: "Elige una categoría", cards: "tarjetas", seconds: "segundos", start: "Empezar", loading: "Cargando…", rotate: "Pon el teléfono horizontal y llévalo a tu frente", correct: "¡Correcto!", pass: "Pasar", direction: "Arriba pasa · abajo acierta", time: "Tiempo", results: "Ronda terminada", again: "Otra ronda", denied: "El sensor necesita permiso para comenzar.", empty: "Esta categoría todavía no tiene opciones.", unavailable: "No pudimos cargar el catálogo. Intenta otra vez." },
  en: { title: "On your forehead", choose: "Choose a category", cards: "cards", seconds: "seconds", start: "Start", loading: "Loading…", rotate: "Turn the phone sideways and place it on your forehead", correct: "Correct!", pass: "Pass", direction: "Tilt up to pass · down for correct", time: "Time", results: "Round over", again: "Play again", denied: "Motion permission is required to begin.", empty: "This category does not have options yet.", unavailable: "The catalog could not be loaded. Try again." },
};
...
export default function HeadsUpGame() {
  ...
  const { permission, requestPermission } = useTiltGesture({ enabled: phase === "playing", onGesture: handleGesture });
  ...
  if (phase === "playing") return <main className={`min-h-dvh ${accent} text-stone-950 transition-colors duration-200 grid grid-rows-[auto_1fr_auto] p-5`}><header className="flex items-center justify-between text-xl font-black"><span>{selected?.name}</span><span className="rounded-full bg-stone-950 px-5 py-2 text-white tabular-nums">{remaining}s</span></header><section className="grid place-items-center"><div className="max-w-5xl text-center text-6xl font-black leading-none sm:text-8xl md:text-9xl">{feedback ? (feedback === "correct" ? copy.correct : copy.pass) : current?.text}</div></section><footer className="flex justify-between text-sm font-black uppercase tracking-[.25em]"><span>↑ {copy.pass}</span><span>{score}</span><span>{copy.correct} ↓</span></footer></main>;
  if (phase === "results") return <main className="min-h-dvh bg-stone-950 p-6 text-white"><div className="mx-auto max-w-3xl pt-16"><p className="text-sm font-black uppercase tracking-[.3em] text-amber-300">{copy.results}</p><div className="my-8 text-9xl font-black text-amber-300">{score}</div><div className="grid gap-3 sm:grid-cols-2">{round?.correct.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl bg-emerald-500/10 p-4 text-emerald-300"><Check />{item.text}</div>)}{round?.passed.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl bg-rose-500/10 p-4 text-rose-300"><X />{item.text}</div>)}</div><Button onClick={() => setPhase("setup")} className="mt-10 h-14 w-full rounded-2xl bg-amber-300 text-lg font-black text-stone-950"><RotateCcw />{copy.again}</Button></div></main>;
