...
export default function HeadsUpGame() {
  ...
  if (phase === "results") return <main className="min-h-dvh bg-stone-950 p-6 text-white"><div className="mx-auto max-w-3xl pt-16"><p className="text-sm font-black uppercase tracking-[.3em] text-amber-300">{copy.results}</p><div className="my-8 text-9xl font-black text-amber-300 tabular-nums">{score}</div><div className="grid gap-3">{round?.results.map((result, index) => { const isCorrect = result.outcome === "correct"; return <button key={`${result.option.id}-${index}`} type="button" aria-label={`${isCorrect ? copy.markMiss : copy.markCorrect}: ${result.option.text}`} aria-pressed={isCorrect} onClick={() => setRound((currentRound) => toggleResult(currentRound, index))} className={`flex min-h-16 w-full items-center gap-4 rounded-2xl border border-l-8 p-4 text-left font-black transition-transform duration-100 active:scale-[.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-300/60 ${isCorrect ? "border-emerald-300 border-l-emerald-100 bg-emerald-800 text-white hover:bg-emerald-700" : "border-rose-300 border-l-rose-800 bg-rose-200 text-rose-950 hover:bg-rose-300"}`}><span className="w-8 shrink-0 text-center font-mono text-sm opacity-60">{index + 1}</span><span className={`grid size-10 shrink-0 place-items-center rounded-full ${isCorrect ? "bg-white text-emerald-800" : "bg-rose-900 text-white"}`}>{isCorrect ? <Check /> : <X />}</span><span className="min-w-0 flex-1 text-lg">{result.option.text}</span><span className="text-xs uppercase tracking-wider opacity-80">{isCorrect ? copy.correct : copy.miss}</span></button>; })}</div><Button onClick={() => setPhase("setup")} className="mt-10 h-14 w-full rounded-2xl bg-amber-300 text-lg font-black text-stone-950"><RotateCcw />{copy.again}</Button></div></main>;
